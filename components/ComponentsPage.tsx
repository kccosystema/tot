...
const ComponentsPage: React.FC<ComponentsPageProps> = ({ onAdd, currentInputIds }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterTier, setFilterTier] = useState<number | 'All'>('All');
  const [sortConfig, setSortConfig] = useState<{ key: keyof ComponentItem; direction: 'asc' | 'desc' } | null>(null);
  ...
  const requestSort = (key: keyof ComponentItem) => {
  ...
  const getSortIndicator = (key: keyof ComponentItem) => {
        ...
        <div className="flex-grow relative">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <svg className="h-5 w-5 text-slate-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M2.5 3A1.5 1.5 0 001 4.5v.793c.026.009.051.02.076.032L7.674 8.51c.206.1.446.1.622 0l6.598-3.185A.755.755 0 0115 5.293V4.5A1.5 1.5 0 0013.5 3h-11zM15 6.913l-6.387 3.185c-.616.305-1.409.305-2.026 0L1 6.913V13.5A1.5 1.5 0 002.5 15h11a1.5 1.5 0 001.5-1.5V6.913z" clipRule="evenodd" />
            </svg>
          </div>
          <input
            type="text"
