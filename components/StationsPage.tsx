...
interface StationsPageProps {
  onAdd: (station: StationItem) => void;
  currentInputIds: Set<string>;
}

const StationsPage: React.FC<StationsPageProps> = ({ onAdd, currentInputIds }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterTier, setFilterTier] = useState<number | 'All'>('All');
  const [sortConfig, setSortConfig] = useState<{ key: keyof StationItem; direction: 'asc' | 'desc' } | null>(null);

  const filteredStations = useMemo(() => {
    return STATION_DATA.filter(s => {
  ...
  const requestSort = (key: keyof StationItem) => {
  ...
  const getSortIndicator = (key: keyof StationItem) => {
