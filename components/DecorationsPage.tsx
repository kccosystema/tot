...
import { DECORATION_DATA } from '../data/decorations';
import { DecorationItem } from '../types';

interface DecorationsPageProps {
  onAdd: (decoration: DecorationItem) => void;
  currentInputIds: Set<string>;
}

const DecorationsPage: React.FC<DecorationsPageProps> = ({ onAdd, currentInputIds }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterTier, setFilterTier] = useState<number | 'All'>('All');
  const [sortConfig, setSortConfig] = useState<{ key: keyof DecorationItem; direction: 'asc' | 'desc' } | null>(null);

  const filteredDecorations = useMemo(() => {
    return DECORATION_DATA.filter(d => {
  ...
  const requestSort = (key: keyof DecorationItem) => {
  ...
  const getSortIndicator = (key: keyof DecorationItem) => {
      ...
      <div className="bg-slate-900 border border-slate-700 p-4 rounded-lg flex flex-col md:flex-row gap-4">
        <div className="flex-grow relative">
          ...
          <input
            ...
            placeholder="Search decorations by name or ID..."
            value={searchTerm}
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-lg overflow-hidden shadow-xl">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left text-slate-400">
            ...
            <tbody>
              {filteredDecorations.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className={`px-4 py-3 text-right font-mono ${hasDurability ? 'text-emerald-400' : 'text-slate-600'}`}>
                      {item.durability <= 0 ? (item.durability === -1 ? '∞' : '-') : item.durability}
                    </td>
              ...
              {filteredDecorations.length === 0 && (
                <tr>
                  <td colSpan={7} className="text-center py-12 text-slate-500 italic">
                    No decoration items found matching your criteria.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="bg-slate-800 px-4 py-2 border-t border-slate-700 text-xs text-slate-500 flex justify-between items-center">
            <span>Showing {filteredDecorations.length} decorations</span>
