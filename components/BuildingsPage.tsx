...
interface BuildingsPageProps {
  onAdd: (building: BuildingItem) => void;
  currentInputIds: Set<string>;
}

const BuildingsPage: React.FC<BuildingsPageProps> = ({ onAdd, currentInputIds }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterTier, setFilterTier] = useState<number | 'All'>('All');
  const [sortConfig, setSortConfig] = useState<{ key: keyof BuildingItem; direction: 'asc' | 'desc' } | null>(null);

  const filteredBuildings = useMemo(() => {
    return BUILDING_DATA.filter(b => {
  ...
  const requestSort = (key: keyof BuildingItem) => {
  ...
  const getSortIndicator = (key: keyof BuildingItem) => {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-lg overflow-hidden shadow-xl">
                ...
                return (
                  ...
                  >
                    ...
                    <td className={`px-4 py-3 text-right font-mono ${hasDurability ? 'text-emerald-400' : 'text-slate-600'}`}>
                      {item.durability === -1 ? '∞' : item.durability}
                    </td>
        ...
        <div className="bg-slate-800 px-4 py-2 border-t border-slate-700 text-xs text-slate-500 flex justify-between items-center">
            <span>Showing {filteredBuildings.length} building items</span>
