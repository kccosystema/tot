...
import { CONSUMABLE_DATA } from '../data/consumables';
import { ConsumableItem } from '../types';

interface ConsumablesPageProps {
  onAdd: (consumable: ConsumableItem) => void;
  currentInputIds: Set<string>;
}

const ConsumablesPage: React.FC<ConsumablesPageProps> = ({ onAdd, currentInputIds }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterTier, setFilterTier] = useState<number | 'All'>('All');
  const [sortConfig, setSortConfig] = useState<{ key: keyof ConsumableItem; direction: 'asc' | 'desc' } | null>(null);

  const filteredConsumables = useMemo(() => {
    return CONSUMABLE_DATA.filter(c => {
  ...
  const requestSort = (key: keyof ConsumableItem) => {
  ...
  const getSortIndicator = (key: keyof ConsumableItem) => {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="bg-slate-900 border border-slate-700 p-4 rounded-lg flex flex-col md:flex-row gap-4">
        <div className="flex-grow relative">
          ...
          <input
            ...
            placeholder="Search consumables by name or ID..."
            value={searchTerm}
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-lg overflow-hidden shadow-xl">
        ...
        <div className="bg-slate-800 px-4 py-2 border-t border-slate-700 text-xs text-slate-500 flex justify-between items-center">
            <span>Showing {filteredConsumables.length} consumables</span>
