...
interface UtilityPageProps {
  onAdd: (utility: UtilityItem) => void;
  currentInputIds: Set<string>;
}

const UtilityPage: React.FC<UtilityPageProps> = ({ onAdd, currentInputIds }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterTier, setFilterTier] = useState<number | 'All'>('All');
  const [sortConfig, setSortConfig] = useState<{ key: keyof UtilityItem; direction: 'asc' | 'desc' } | null>(null);

  const filteredUtility = useMemo(() => {
    return UTILITY_DATA.filter(u => {
  ...
  const requestSort = (key: keyof UtilityItem) => {
  ...
  const getSortIndicator = (key: keyof UtilityItem) => {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="bg-slate-900 border border-slate-700 p-4 rounded-lg flex flex-col md:flex-row gap-4">
        <div className="flex-grow relative">
          ...
          <input
            ...
            placeholder="Search utility items by name or ID..."
            value={searchTerm}
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-lg overflow-hidden shadow-xl">
              ...
              {filteredUtility.length === 0 && (
                <tr>
                  <td colSpan={7} className="text-center py-12 text-slate-500 italic">
                    No utility items found matching your criteria.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="bg-slate-800 px-4 py-2 border-t border-slate-700 text-xs text-slate-500 flex justify-between items-center">
            <span>Showing {filteredUtility.length} utility items</span>
