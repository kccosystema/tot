...
interface ArmorPageProps {
  onAdd: (armor: ArmorItem) => void;
  currentInputIds: Set<string>;
}

const ArmorPage: React.FC<ArmorPageProps> = ({ onAdd, currentInputIds }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterTier, setFilterTier] = useState<number | 'All'>('All');
  const [sortConfig, setSortConfig] = useState<{ key: keyof ArmorItem; direction: 'asc' | 'desc' } | null>(null);

  const filteredArmor = useMemo(() => {
    return ARMOR_DATA.filter(a => {
  ...
  const requestSort = (key: keyof ArmorItem) => {
  ...
  const getSortIndicator = (key: keyof ArmorItem) => {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="bg-slate-900 border border-slate-700 p-4 rounded-lg flex flex-col md:flex-row gap-4">
        <div className="flex-grow relative">
          ...
          <input
            ...
            placeholder="Search armor by name or ID..."
            value={searchTerm}
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-lg overflow-hidden shadow-xl">
          ...
          <table className="w-full text-sm text-left text-slate-400">
            <thead className="text-xs text-slate-200 uppercase bg-slate-800 border-b border-slate-700 sticky top-0 z-10">
              <tr>
                ...
                <th scope="col" className="px-4 py-3 cursor-pointer hover:bg-slate-700" onClick={() => requestSort('category')}>
                  Category {getSortIndicator('category')}
                </th>
                <th scope="col" className="px-4 py-3 cursor-pointer hover:bg-slate-700 text-right" onClick={() => requestSort('armorValue')}>
                  Armor {getSortIndicator('armorValue')}
                </th>
              ...
              {filteredArmor.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right font-mono text-blue-400">{item.armorValue}</td>
                    <td className="px-4 py-3 text-right font-mono">{item.durability === -1 ? '∞' : item.durability}</td>
        ...
        <div className="bg-slate-800 px-4 py-2 border-t border-slate-700 text-xs text-slate-500 flex justify-between items-center">
            <span>Showing {filteredArmor.length} items</span>
            <span>Items added will use Armor Value for pricing calculation.</span>
        </div>
      </div>
    </div>
  );
